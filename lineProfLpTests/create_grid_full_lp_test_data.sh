...
diskThicknessParamFile="${inPath}/full_lp_thickdisk_thickness_params.txt"
...
while INF=' ' read -ra spinLine; do
	...
	while INF=' ' read -ra inclinationLine; do
		...
		while INF=' ' read -ra heightLine; do
			...
			while INF=' ' read -ra thicknessLine; do
                diskThickness=${thicknessLine[0]}

				qdpFile="${outPath}/lp_grid_disk${diskCombinationIndex}_lp${lpCombinationIndex}_thickness${diskThicknessIndex}.qdp"
				txtFile="${outPath}/lp_grid_disk${diskCombinationIndex}_lp${lpCombinationIndex}_thickness${diskThicknessIndex}.txt"
				npyFile="${outPath}/lp_grid_disk${diskCombinationIndex}_lp${lpCombinationIndex}_thickness${diskThicknessIndex}.npy"
				...
				pl eem
				ipl
				wdata ${outPath}/lp_grid_disk${diskCombinationIndex}_lp${lpCombinationIndex}_thickness${diskThicknessIndex}
				exit
				exit
				EOF
