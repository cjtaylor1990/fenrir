...
inFile1 = filePath + 'model_relfen.npy'#'#myrazor_test11_newscript.npy'#'corona_a0998_acc0_r10_onaxis_n1000_tol500_hist_test.npy'
inFile2 = filePath + 'model_relxill.npy'#'./pvc_cart_n1000_spec.npy'#./relline_test9+10+11_newmod.npy'
...
outFile = '/Users/cjtaylor/Desktop/compare_models_new.pdf'
...
pl.plot(dataEnergy1, (dataFlux1)/np.sum(dataFlux1*(dataEnergy1[1]-dataEnergy1[0])), color = 'k', linestyle = '-')#(plotDataFlux)/dataSum
pl.plot(dataEnergy2, dataFlux2/np.sum(dataFlux2*(dataEnergy2[1]-dataEnergy2[0])), color = 'r', linestyle = '-')
pl.plot(dataEnergy1, (dataFlux1/np.sum(dataFlux1*(dataEnergy1[1]-dataEnergy1[0])))-(dataFlux2/np.sum(dataFlux2*(dataEnergy2[1]-dataEnergy2[0]))), color = 'b', linestyle = '-')
