...
while INF=' ' read -ra line; do
	...
	while [ $dummy_i -lt $num_procs ];do
		./test.out $photons_per_side $num_procs $dummy_i "$output_path/test_$dummy_i.txt" $spin_val $inc_val 0.0 70 &
		dummy_i=$((dummy_i+1))
		pids+=($!)
	done
	wait ${pids[*]}
	...
	python3 txt_to_npy_diskimg.py $combine_output $combine_npy
	python3 transfer_auto_single2.py $spin_val $combine_npy $fits_output
	rm -f $combine_npy
done < $param_file