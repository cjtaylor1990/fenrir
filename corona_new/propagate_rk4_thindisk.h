...
void propagate(double posVec[4], double momVec[4], double dStep, double tolerance, double maxStep, double rLimitLow,double rLimitHigh, double rEvent, double scaleHeightValue, double rProjected){
	...
	while ((i < maxStep) && (radius < rLimitHigh) && (radius > rLimitLow) && (radius*cos(theta) > scaleHeightFnct(radius,theta))){
    ...
    //If photon hit disk, change the switch to 1 from 0
    if (!(radius*cos(theta) > scaleHeightFnct(radius,theta))){
        hitDiskSwitch = 1;
    }
